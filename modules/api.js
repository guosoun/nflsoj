...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (!user) throw 1001;
    else if (user.isEnded ()) throw 2022;
    ...
    else if (user.group_id == null || user.group_id === '0') res.send({ error_code: 2020});
    else {
  ...
  try {
    ...
    if (syzoj.config.register_mail) {
      ...
      user = await User.create({
        ...
        nickname: req.body.nickname,
        password: req.body.password,
        luogu_account: req.body.luogu_account,
        codeforces_account: req.body.codeforces_account,
        atcoder_account: req.body.atcoder_account,
        email: req.body.email,
